(function () {
  ...
  function showCareerError() {
    ...
    if (grid) {
      grid.innerHTML = window.Arivuu.renderSheetDataError
        ? window.Arivuu.renderSheetDataError({ retryId: 'career-library-retry', fullWidth: true })
        : '<div class="career-library-error"><p>Couldn\'t load data at the moment. Please try again later.</p></div>';
  ...
  function loadCareerLibrary() {
    ...
    return window.Arivuu.loadCareerData().then(function (data) {
      if (!data || !data.careers || !data.careers.length) {
  ...
  function showBlogError(retryHandler) {
    var grid = document.getElementById('blog-grid');
    if (!grid) return;
    var retryId = retryHandler ? 'blog-data-retry' : '';
    grid.innerHTML = window.Arivuu.renderSheetDataError
      ? window.Arivuu.renderSheetDataError({ retryId: retryId, fullWidth: true })
      : '<div class="col-span-full text-center py-16"><p class="text-muted-text text-sm">Couldn\'t load data at the moment. Please try again later.</p></div>';
    if (retryHandler) {
      var retryBtn = document.getElementById('blog-data-retry');
      if (retryBtn) retryBtn.addEventListener('click', retryHandler);
    }
  }

  function loadBlogDataForPreview() {
    return window.Arivuu.loadBlogData().then(function (data) {
      ...
      showBlogError(loadBlogDataForPreview);
    });
  }

  function loadBlogDataForPage() {
    return window.Arivuu.loadBlogData().then(function (data) {
      if (!data || !data.posts || !data.posts.length) {
        throw new Error('No blog posts found');
      }
      ...
      showBlogError(loadBlogDataForPage);
    });
  }

  function initHome() {
    loadCareerLibrary();
    loadBlogDataForPreview();
  }
  ...
  function initBlogPostPage() {
    ...
    window.Arivuu.loadBlogData().then(function (data) {
      ...
      if (article) {
        article.innerHTML = window.Arivuu.renderSheetDataError
          ? window.Arivuu.renderSheetDataError({ retryId: 'blog-post-retry' })
          : '<div class="text-center py-16"><p class="text-muted-text">Couldn\'t load data at the moment. Please try again later.</p></div>';
        var retryBtn = document.getElementById('blog-post-retry');
        if (retryBtn) {
          retryBtn.addEventListener('click', function () {
            initBlogPostPage();
          });
